---
name: resolution-playbook-synthesis
description: Generating verified step-by-step diagnostic procedures matching the exact customer error codes.
---

# resolution-playbook-synthesis

## Procedure
1. Parse incoming domain input and identify critical parameters.
2. Cross-reference with standard operational baselines.
3. Perform step-by-step evaluation using first-principles logic.
4. Verify edge cases and consistency constraints.
5. Emit structured JSON output report.

## Output Contract
```json
{
  "skill": "resolution-playbook-synthesis",
  "status": "completed",
  "confidence": 0.95,
  "findings": []
}
```
